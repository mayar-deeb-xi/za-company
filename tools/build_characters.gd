...
## regenerated character works headless immediately with no --import pass.
##
...
# Animation layout: row indices in the sheet, verified against it. The sheet
# only draws a right-facing profile; "side" is flipped for left.
const LAYOUT := {
...
func _texture_curls(img: Image, ox: int, oy: int, hair_hex: String, light_hex: String) -> void:
...
func _add_beard(img: Image, ox: int, oy: int) -> void:
...
func _body_bounds(img: Image, ox: int, oy: int) -> Rect2i:
...
func _seam_x(img: Image, ox: int, oy: int) -> int:
...
func _widen(img: Image, ox: int, oy: int) -> void:
	for y in FRAME:
...
func _narrow(img: Image, ox: int, oy: int) -> void:
	var seam := _seam_x(img, ox, oy)
	if seam < 0:
		return
	for y in FRAME:
...
func _heighten(img: Image, ox: int, oy: int) -> void:
	for x in FRAME:
...
func _build_sheet(recipe: Dictionary) -> Image:
...
func _build_frames(img: Image) -> SpriteFrames:
...
func _initialize() -> void:
	if Image.load_from_file(ProjectSettings.globalize_path(SRC)) == null:
